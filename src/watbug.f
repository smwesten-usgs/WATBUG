      ...
      dimension T(481),P(481),PE(481),APE(481),D(481),MON(481),LABEL(18),
...
c
c     'TUNIT'  Designates the units of air temperature. 1.0 means
c              the raw temperature data are in degrees Fahrenheit.
c              2.0 means degrees Kelvin. Any other numeric
c              designation or a blank means degrees Celsius.
c
c     'PUNIT'  Units of precipitation. 1.0 means the raw data
c              are in centimeters. 2.0 means inches. 3.0 means hundreths
...
c*************************************************************************
     ...
     1     ST(M),DST(M),DEF(M),SUR(M),MON(M),FMT(18),LABEL(18)
...
c
      ...
      C2 = 1.0
      FK = 0.0
      if (TUNIT .eq. 1.0)  FK = 32
      if (TUNIT .eq. 1.0)  C1 = 5.0/9.0
      if (TUNIT .eq. 2.0)  FK = 273.16
      if (PUNIT .eq. 1.0)  C2 = 10.0
      if (PUNIT .eq. 2.0)  C2 = 25.4
      if (PUNIT .eq. 3.0)  C2 = 0.254
...
c***
      ...
      call bal(N,M,ST,D,FC,SM,SUR,DST,DT,DAYS,KM)
...
c*************************************************************************
      subroutine bal(N,M,ST,D,FC,SM,SUR,DST,DT,DAYS,KM)

        real ST(M),D(M),SUR(M),DST(M),DAYS(13)
